...
const getCurrentDomainConfig = (config) =>
  new Promise((resolve, reject) => {
    axios
      .post(config.admURL, {
...
const updateDomainConfig = (config, ip, records) =>
  new Promise((resolve, reject) => {
...
const getListOfRecordsToUpdate = (config, ip, domainConfig) => {
  const res = [];
...
module.exports = (config) =>
  Promise
    .all([
      getCurrentIP(),
      getCurrentDomainConfig(config)
    ])
    .then(response => {
      const ip = response[0];
      const domainConfig = response[1].data;

      const recordsToUpdate = getListOfRecordsToUpdate(config, ip, domainConfig);

      if (recordsToUpdate.length) {
        console.log('Going to update IP for this records: \n');
        console.log(recordsToUpdate);
        return updateDomainConfig(config, ip, recordsToUpdate);
      } else {
